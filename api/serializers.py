...
class OrderCreateSerializers(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        orderitem_date = validated_data.pop('items')
        instance = super().update(instance , validated_data)
        with transaction.atomic():
         instance = super.update(instance , validated_data)
        if orderitem_date is not None :
            instance.items.all.dalete()
            for item in orderitem_date :
...
class OrderSerializers(serializers.ModelSerializer):
    ...
    def total(self, obj):
        ...
        return sum(item.item_subotal for item in order_items)
